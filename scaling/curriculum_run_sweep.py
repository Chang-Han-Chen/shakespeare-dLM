"""Run p_ar curricula using the selected LR from each pure-BD IsoFLOP point."""
...
def parse_args():
    ...
    parser.add_argument("--p-ar", type=float, choices=P_AR_VALUES, action="append")
...
def baseline_learning_rate(budget, spec) -> float:
    """Read the locally bracketed LR selected in the pure-BD sweep."""
    path = (
        BASE_RESULTS_DIR
        / "runs"
        / budget_slug(budget)
    ...
    if not path.exists():
        raise RuntimeError(
            f"No prior LR selection for C={budget:.0e}, N={spec.label}: {path}"
        )
    bracket = json.loads(path.read_text(encoding="utf-8"))
    if bracket.get("status") != "locally_bracketed":
        raise RuntimeError(f"Prior LR is not locally bracketed: {path}")
...
def main() -> None:
    ...
    for run in runs:
        print(
            f"p={run['p_ar']:.1f} {budget_slug(run['budget']):>5} "
            f"{run['spec'].label:>5} steps={run['steps']:>6} "
            f"({run['ar_steps']}+{run['bd_steps']}) lr={run['lr']:.4g}"
        )
    if args.dry_run:
        return
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for index, run in enumerate(todo, start=1):
            print(
                f"[{index}/{len(todo)}] queue p={run['p_ar']:.1f} "
                f"C={run['budget']:.0e} N={run['spec'].label} "
                f"lr={run['lr']:.4g} steps={run['steps']}",
                flush=True,
            )
        ...
        for future in as_completed(futures):
            ...
            print(
                f"[{completed}/{len(todo)} complete] p={run['p_ar']:.1f} "
                f"C={run['budget']:.0e} N={run['spec'].label} "
                f"lr={run['lr']:.4g} val={result['val_nelbo']:.5f} "
