...
Uses the best (model, size) → LR mapping found by sweep.py, all at bs=128.
Runs 4000 steps with GPT2 eval enabled for apples-to-apples comparison.

Results saved to scaling/{model}/{size}/loss.pkl

Usage:
    python run_scaling.py                          # all 20 runs sequentially
...
# ---------------------------------------------------------------
# Optimal LR per (model, size) from sweep — all at bs=128
# ---------------------------------------------------------------
...
# Best LR per (model, size) from 800-step sweep
OPTIMAL_LR = {
    ("remasked",      "0.1M"): 1e-2,
    ("remasked",      "0.3M"): 3e-3,
    ("remasked",      "1M"):   3e-3,
...
def run_one(config):
    ...
    n_embd, n_layer = MODEL_SIZES[size]
    lr = OPTIMAL_LR[(model, size)]
    out_dir = f"scaling/{model}/{size}"
    os.makedirs(out_dir, exist_ok=True)

    cmd = build_command(model, size, out_dir)
    label = f"{model} {size} lr={lr:.0e}"
...
def print_summary_table(results):
    ...
    print("\n" + "=" * 105)
    print("SCALING RESULTS (4000 steps, bs=128, optimal LR per config)")
    print("=" * 105)
