...
class Model(nn.Module):
    ...
    def forward(self, idx, targets=None, supervise_mask=None):
        ...
        Parameters
        ----------
        idx            : (B, L) token ids (may include corrupted visible tokens)
        targets        : (B, L) ground-truth token ids
        supervise_mask : (B, L) bool — True for positions the loss should
                         cover (masked positions AND corrupted visible positions)
        """
        B, Tseq = idx.size()
        ...
        if targets is not None:
            ...
            loss = (per_token_loss * supervise_f).sum() / supervise_f.sum().clamp_min(1.0)
